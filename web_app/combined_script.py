                ...
                try:
                    ...
                    if len(dims) >= 2:
                        ...
                        if w > h:
                            ...
                            print(f"[DEBUG] Found horizontal dimensions: {h} x {w}")
...
def generate_final_product_list(user_data: Dict[str, Any]) -> Dict[str, Any]:
    ...
    # Initialize final_list structure
    final_list = {
        'hall': {'fans': [], 'ac': []},
        'master_bedroom': {'fans': [], 'ac': []},
        'bedroom_2': {'fans': [], 'ac': []},
        # 'bedroom_3': {'fans': [], 'ac': []},
        'dining': {'fans': [], 'ac': []},
        'kitchen': {'chimney': [], 'gas_stove': [], 'small_fan': []},
        'laundry': {'washing_machine': [], 'dryer': []},
        'summary': {
            'total_budget': user_data['total_budget'],
            'family_size': user_data['demographics'],
    ...
    if(num_bedrooms == 3):
        print("3 bedrooms")
        final_list['bedroom_3'] = {'fans': [], 'ac': []}
    else:
        print("Not 3 bedrooms")
    ...
    if user_data['master_bedroom'].get('fans', 0) > 0:
        budget_category = get_budget_category(user_data['total_budget'], 'ceiling_fan')  # Single fan for master bedroom
    ...
    if user_data['bedroom_2'].get('fans', 0) > 0:
        budget_category = get_budget_category(user_data['total_budget'], 'ceiling_fan')  # Single fan for bedroom 2
    ...
    num_bedrooms = int(user_data.get('num_bedrooms', user_data.get('Number of bedrooms', 2)))
    if(num_bedrooms == 3):
        if user_data['bedroom_3'].get('fans', 0) > 0:
            budget_category = get_budget_category(user_data['total_budget'], 'ceiling_fan')  # Single fan for bedroom 3
    ...
    # Process dining requirements
    if user_data['dining'].get('fans', 0) > 0:
        budget_category = get_budget_category(user_data['total_budget'], 'ceiling_fan')  # Single fan for dining
        recommendations = get_specific_product_recommendations('ceiling_fan', budget_category, user_data['demographics'], user_data['dining'].get('color_theme'), user_data)
...
######
    ...
    num_bedrooms = int(user_data.get('num_bedrooms', user_data.get('Number of bedrooms', 2)))
    if(num_bedrooms == 3):
        print("3 bedrooms, so getting into br3")
    ...
    # Process bedroom 3 bathroom requirements
        ...
        if str(bedroom3_bath_type).strip().lower() == 'yes':
            ...
            if ceiling_condition == 'yes':
                ...
                print(f"[DEBUG] Found horizontal geyers : {len(horizontal_geysers)} horizontal geysers")
            ...
            if isinstance(p.get("product_type", ""), str):
                ...
                # Special case: geyser can be labeled as 'geyser', 'water heater', 'instant water heater', or 'storage water heater'
                    ...
                    if matches and required_features.get('water_heater_ceiling', '').lower() == 'yes':
                        ...
                        # print(f"[DEBUG][Geyser] After orientation check: {matches}")
                # Special case: bathroom_exhaust can be labeled as 'exhaust fan', 'bathroom exhaust', or 'ventilation fan'
                elif norm_type == 'bathroom_exhaust':
                    matches = any(t in product_type_norm for t in ['exhaust fan', 'bathroom exhaust', 'ventilation fan'])
                else:
                    matches = product_type_norm == norm_type
                if matches:
...
######
    # Process bedroom 3 requirements
    num_bedrooms = int(user_data.get('num_bedrooms', user_data.get('Number of bedrooms', 2)))
    print(f"[DEBUG] Number of bedrooms: {num_bedrooms}")
    if num_bedrooms == 3:
        if user_data['bedroom_3'].get('ac', False):
            # print(f"[DEBUG] Bedroom 2 AC requirement: {user_data['bedroom_2'].get('ac', False)}") # DEBUG
            budget_category = get_budget_category(user_data['total_budget'], 'ac')
